...
begin
  ...
  try
    ...
    try
      ...
      // --> Record Count
      OutStream.PutLongInt(Length(EntryList) - 1);
      // --> Table Offset
...
begin
  Result := CompareText(StdString(EntryList[Value1].Key), StdString(EntryList[Value2].Key));
end;
...
begin
  ...
  begin
    Mid := (Lo + Hi) div 2;
    Res := CompareText(StdString(EntryList[SearchList[Mid]].Key), StdString(Key));
